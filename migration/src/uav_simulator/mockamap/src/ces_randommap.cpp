...
// rclcpp::TimerBase::SharedPtr vis_map;
bool map_ok    = false;
...
rclcpp::Time                      begin_time = rclcpp::Time(RCL_ROS_TIME, 0x7FFFFFFFFFFFFFFFLL);
...
fixedMapGenerate()
{
  ...
  RCLCPP_WARN(rclcpp::get_logger("mockamap"), "Finished generate random map ");
...
void
rcvOdometryCallbck(const nav_msgs::msg::Odometry odom)
{
...
publishAllPoints()
{
  ...
  if ((rclcpp::Clock(RCL_ROS_TIME).now() - begin_time).seconds() > 7.0)
    return;
  ...
  if (frequence_division_global == 0)
  {
    ...
    if (_global_map_pub) _global_map_pub->publish(globalMap_pcd);
    frequence_division_global = 40;
    RCLCPP_ERROR(rclcpp::get_logger("mockamap"), "[SERVER]Publish one global map");
  }
}
...
pubSensedPoints()
{
  ...
  {
    // RCLCPP_ERROR(rclcpp::get_logger("mockamap"), "[Map server] No obstacles .");
  ...
  if (_local_map_pub) _local_map_pub->publish(localMap_pcd);

  rclcpp::Time time_aft_sensing = rclcpp::Clock(RCL_ROS_TIME).now();
  ...
  if (frequence_division_global == 0)
  {
    ...
    if (_global_map_pub) _global_map_pub->publish(globalMap_pcd);
    frequence_division_global = 40;
    RCLCPP_INFO(rclcpp::get_logger("mockamap"), "[SERVER]Publish one global map");
  }
}
