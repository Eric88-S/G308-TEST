...
Maps::randomMapGenerate()
{
  ...
  _w_l = info.nh_private->declare_parameter<double>("width_min", 0.6);
  _w_h = info.nh_private->declare_parameter<double>("width_max", 1.5);
  _ObsNum = info.nh_private->declare_parameter<int>("obstacle_number", 10);
...
Maps::pcl2ros()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("mockamap"), "finish: infill %lf%%",
...
Maps::perlin3D()
{
  ...
  complexity = info.nh_private->declare_parameter<double>("complexity", 0.142857);
  fill = info.nh_private->declare_parameter<double>("fill", 0.38);
  fractal = info.nh_private->declare_parameter<int>("fractal", 1);
  attenuation = info.nh_private->declare_parameter<double>("attenuation", 0.5);
  ...
  RCLCPP_INFO(rclcpp::get_logger("mockamap"), "threshold: %lf", tmp);
  ...
  RCLCPP_INFO(rclcpp::get_logger("mockamap"), "the number of points before optimization is %d", info.cloud->width);
  info.cloud->points.resize(info.cloud->width * info.cloud->height);
  pcl2ros();
}
...
Maps::recursiveDivision(int xl, int xh, int yl, int yh, Eigen::MatrixXi& maze)
{
  ...
  { // the remaining area is larger than or equal to 5*5, need to add both x
    ...
    int  xm    = 0;
    int  ym    = 0;
    RCLCPP_INFO(rclcpp::get_logger("mockamap"), "entered 5*5 mode");
    while (valid == false)
    {
    ...
    RCLCPP_INFO(rclcpp::get_logger("mockamap"), "finished generating maze with width %d , height %d",
             xh - xl + 1,
             yh - yl + 1);
    std::cout << maze << std::endl;
    return;
  ...
  { // the case of 3*4+
    RCLCPP_INFO(rclcpp::get_logger("mockamap"), "entered 3*4+ mode");
    int doorcount = 0;
    int ym        = 0;
    for (int i = yl; i <= yh; i++)
    {
  ...
  { // the case of 4+*3
    RCLCPP_INFO(rclcpp::get_logger("mockamap"), "entered 4+*3 mode");
    int doorcount = 0;
    int xm        = 0;
    for (int i = xl; i <= xh; i++)
    {
...
Maps::recursizeDivisionMaze(Eigen::MatrixXi& maze)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("mockamap"), "debug %d %d %d %d", sx, sy, px, py);
  ...
  if ((sy - py - 3) > 0)
    ...
    y2 = py + 1;
  RCLCPP_INFO(rclcpp::get_logger("mockamap"), "%d %d %d %d", x1, x2, y1, y2);
...
Maps::maze2D()
{
  ...
  int    addWallX;
  int    addWallY;
  width = info.nh_private->declare_parameter<double>("road_width", 1.0);
  addWallX = info.nh_private->declare_parameter<int>("add_wall_x", 0);
  addWallY = info.nh_private->declare_parameter<int>("add_wall_y", 0);
  type = info.nh_private->declare_parameter<int>("maze_type", 1);
...
Maps::Maze3DGen()
{
  ...
  numNodes = info.nh_private->declare_parameter<int>("numNodes", 10);
  connectivity = info.nh_private->declare_parameter<double>("connectivity", 0.5);
  nodeRad = info.nh_private->declare_parameter<int>("nodeRad", 3);
  roadRad = info.nh_private->declare_parameter<int>("roadRad", 2);
  RCLCPP_INFO(rclcpp::get_logger("mockamap"), "received parameters : numNodes: %d connectivity: "
  ...
  for (int i = 0; i < numNodes; i++)
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("mockamap"), "point: x: %f , y: %f , z: %f", rx, ry, rz);
