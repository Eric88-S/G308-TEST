...
void send_transform(const string& parent, const string& child,
                    const colvec& trans, const colvec& q,
...
void odom_callback(const nav_msgs::msg::Odometry::ConstSharedPtr msg)
{
  ...
  if ((t - pt).seconds() > 0.5)
  {
    ...
    trajROS.header.stamp    = g_node->get_clock()->now();
  ...
  if (tf45)
  {
    ...
    colvec t0 = zeros<colvec>(3);
    send_transform(string("world"), string("base"),   t_base, q,    msg->header.stamp);
    send_transform(string("base"),  string("laser"),  t0,     q45,  msg->header.stamp);
    send_transform(string("base"),  string("vision"), t0,     q45,  msg->header.stamp);
    send_transform(string("base"),  string("height"), t0,     q90,  msg->header.stamp);
  }
}
...
int main(int argc, char** argv)
{
  ...
  mesh_resource = g_node->declare_parameter<std::string>("mesh_resource", std::string("package://odom_visualization/meshes/hummingbird.mesh"));
  color_r = g_node->declare_parameter<double>("color/r", 1.0);
  color_g = g_node->declare_parameter<double>("color/g", 0.0);
  color_b = g_node->declare_parameter<double>("color/b", 0.0);
  color_a = g_node->declare_parameter<double>("color/a", 1.0);
  origin  = g_node->declare_parameter<bool>("origin", false);
  scale   = g_node->declare_parameter<double>("robot_scale", 2.0);
  _frame_id = g_node->declare_parameter<std::string>("frame_id", string("world"));

  cross_config = g_node->declare_parameter<bool>("cross_config", false);
  tf45       = g_node->declare_parameter<bool>("tf45", false);
  cov_scale  = g_node->declare_parameter<double>("covariance_scale", 100.0);
  cov_pos    = g_node->declare_parameter<bool>("covariance_position", false);
  cov_vel    = g_node->declare_parameter<bool>("covariance_velocity", false);
  cov_color  = g_node->declare_parameter<bool>("covariance_color", false);

  auto sub_odom = g_node->create_subscription<nav_msgs::msg::Odometry>(
    "odom", 100, odom_callback);
  auto sub_cmd = g_node->create_subscription<quadrotor_msgs::msg::PositionCommand>(
  ...
  broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(g_node);
