...
#include "multi_map_server/quaternion_utils.h"
...
public:
  ...
  void PackMsg(multi_map_server::msg::SparseMap3D &msg)
  {
    ...
    msg.info.map_load_time      = rclcpp::Clock(RCL_SYSTEM_TIME).now();
    ...
    msg.info.origin.orientation = quaternionFromYaw(0.0);
...
private:
  ...
  void CheckDecayMap()
  {
    ...
    static rclcpp::Time prevDecayT = rclcpp::Clock(RCL_SYSTEM_TIME).now();
