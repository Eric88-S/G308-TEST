...
void rcvGlobalPointCloudCallBack(
  ...
  RCLCPP_WARN(g_node->get_logger(), "Global Pointcloud received..");
...
int main(int argc, char** argv) {
  ...
  g_node = std::make_shared<rclcpp::Node>("pcl_render", options);

  sensing_horizon = g_node->declare_parameter<double>("sensing_horizon", 10.0);
  sensing_rate = g_node->declare_parameter<double>("sensing_rate", 10.0);
  estimation_rate = g_node->declare_parameter<double>("estimation_rate", 10.0);

  _x_size = g_node->declare_parameter<double>("map.x_size", 50.0);
  _y_size = g_node->declare_parameter<double>("map.y_size", 50.0);
  _z_size = g_node->declare_parameter<double>("map.z_size", 5.0);
  _resolution = g_node->declare_parameter<double>("map.resolution", 0.2);

  // subscribe point cloud
  auto global_map_sub = g_node->create_subscription<sensor_msgs::msg::PointCloud2>(
      "global_map", 1, rcvGlobalPointCloudCallBack);
  auto local_map_sub = g_node->create_subscription<sensor_msgs::msg::PointCloud2>(
  ...
  pub_cloud = g_node->create_publisher<sensor_msgs::msg::PointCloud2>(
      "/pcl_render_node/cloud", 10);

  double sensing_duration = 1.0 / sensing_rate * 2.5;

  auto local_sensing_timer = g_node->create_wall_timer(
