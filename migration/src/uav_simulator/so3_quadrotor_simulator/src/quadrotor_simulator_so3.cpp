...
getControl(const QuadrotorSimulator::Quadrotor& quad, const Command& cmd)
{
  ...
  float M1 = -cmd.k_roll[0] * eR1 - cmd.k_omega[0] * eOm1 + in1;
  float M2 = -cmd.k_roll[1] * eR2 - cmd.k_omega[1] * eOm2 + in2;
  float M3 = -cmd.k_roll[2] * eR3 - cmd.k_omega[2] * eOm3 + in3;
...
cmd_callback(const quadrotor_msgs::msg::SO3Command::ConstSharedPtr cmd)
{
  ...
  command.k_roll[0]        = cmd->k_roll[0];
  command.k_roll[1]        = cmd->k_roll[1];
  command.k_roll[2]        = cmd->k_roll[2];
  command.k_omega[0]       = cmd->k_omega[0];
  command.k_omega[1]       = cmd->k_omega[1];
  command.k_omega[2]       = cmd->k_omega[2];
...
main(int argc, char** argv)
{
  ...
  auto n = std::make_shared<rclcpp::Node>("quadrotor_simulator_so3");
  ...
  _init_x = n->declare_parameter<double>("simulator/init_state_x", 0.0);
  _init_y = n->declare_parameter<double>("simulator/init_state_y", 0.0);
  _init_z = n->declare_parameter<double>("simulator/init_state_z", 1.0);

  Eigen::Vector3d position = Eigen::Vector3d(_init_x, _init_y, _init_z);
  quad.setStatePos(position);

  double simulation_rate;
  simulation_rate = n->declare_parameter<double>("rate/simulation", 1000.0);
  assert(simulation_rate > 0);

  double odom_rate;
  odom_rate = n->declare_parameter<double>("rate/odom", 100.0);
  const auto odom_pub_duration = rclcpp::Duration(std::chrono::nanoseconds(static_cast<int64_t>(1e9 / odom_rate)));

  std::string quad_name;
  quad_name = n->declare_parameter<std::string>("quadrotor_name", "quadrotor");
  ...
  rclcpp::Time next_odom_pub_time = n->get_clock()->now();
  while (rclcpp::ok())
  {
    rclcpp::spin_some(n);
    ...
    rclcpp::Time tnow = n->get_clock()->now();
