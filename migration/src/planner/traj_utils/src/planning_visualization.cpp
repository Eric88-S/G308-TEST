...
namespace ego_planner
{
  PlanningVisualization::PlanningVisualization(rclcpp::Node* nh)
  {
    ...
    goal_point_pub_ = nh->create_publisher<visualization_msgs::msg::Marker>("goal_point", 2);
    global_list_pub_ = nh->create_publisher<visualization_msgs::msg::Marker>("global_list", 2);
    init_list_pub_ = nh->create_publisher<visualization_msgs::msg::Marker>("init_list", 2);
    optimal_list_pub_ = nh->create_publisher<visualization_msgs::msg::Marker>("optimal_list", 2);
    a_star_list_pub_ = nh->create_publisher<visualization_msgs::msg::Marker>("a_star_list", 20);
  }
  ...
  void PlanningVisualization::displayGlobalPathList(vector<Eigen::Vector3d> init_pts, const double scale, int id)
  {
    ...
    if (global_list_pub_->get_subscription_count() == 0)
    {
      return;
    }

    Eigen::Vector4d color(0, 0.5, 0.5, 1);
    displayMarkerList(global_list_pub_, init_pts, scale, color, id);
  }

  void PlanningVisualization::displayInitPathList(vector<Eigen::Vector3d> init_pts, const double scale, int id)
  {

    if (init_list_pub_->get_subscription_count() == 0)
    {
      return;
    }

    Eigen::Vector4d color(0, 0, 1, 1);
    displayMarkerList(init_list_pub_, init_pts, scale, color, id);
  }

  void PlanningVisualization::displayOptimalList(Eigen::MatrixXd optimal_pts, int id)
  {

    if (optimal_list_pub_->get_subscription_count() == 0)
    {
      return;
    }
    ...
    displayMarkerList(optimal_list_pub_, list, 0.15, color, id);
  }
  ...
  void PlanningVisualization::displayAStarList(std::vector<std::vector<Eigen::Vector3d>> a_star_paths, int id /* = Eigen::Vector4d(0.5,0.5,0,1)*/)
  {
    ...
    if (a_star_list_pub_->get_subscription_count() == 0)
    {
      return;
    }
    ...
    //   displayMarkerList(a_star_list_pub_, list, scale, color, id+i);
    // }
    ...
    for (auto block : a_star_paths)
    {
      ...
      displayMarkerList(a_star_list_pub_, list, scale, color, id + i); // real ids used: [ id ~ id+a_star_paths.size() ]
      i++;
    }
  }
