...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(rclcpp::Node* nh, PlanningVisualization::Ptr vis)
  {
    ...
    pp_.feasibility_tolerance_ = nh->declare_parameter<double>("manager/feasibility_tolerance", 0.0);
    pp_.ctrl_pt_dist = nh->declare_parameter<double>("manager/control_points_distance", -1.0);
    pp_.planning_horizen_ = nh->declare_parameter<double>("manager/planning_horizon", 5.0);
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    rclcpp::Time t_start = rclcpp::Clock(RCL_ROS_TIME).now();
    ...
    do
    {
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        double t;
        double t_cur = (rclcpp::Clock(RCL_ROS_TIME).now() - local_data_.start_time_).seconds();
    ...
    t_init = rclcpp::Clock(RCL_ROS_TIME).now() - t_start;
    ...
    t_opt = rclcpp::Clock(RCL_ROS_TIME).now() - t_start;
    t_start = rclcpp::Clock(RCL_ROS_TIME).now();
    ...
    t_refine = rclcpp::Clock(RCL_ROS_TIME).now() - t_start;

    // save planned results
    updateTrajInfo(pos, rclcpp::Clock(RCL_ROS_TIME).now());
  ...
  bool EGOPlannerManager::EmergencyStop(Eigen::Vector3d stop_pos)
  {
    ...
    updateTrajInfo(UniformBspline(control_points, 3, 1.0), rclcpp::Clock(RCL_ROS_TIME).now());
