...
def test_predict_txt():
    ...
    assert len(results) == 7  # 1 + YOLO + YOLO + YOLO = 7 images
...
def test_predict_img(model_name):
    ...
    assert len(model(torch.rand((2, channels, 32, 32)), imgsz=32)) == 2  # batch-size YOLO Tensor, FP32 0.0-1.0 RGB order
...
def test_model_embeddings():
    ...
    for batch in [SOURCE], [SOURCE, SOURCE]:  # test batch size 1 and YOLO
