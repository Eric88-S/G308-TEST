...
```python
df = exp.ask_ai("show me images containing more than 10 objects with at least YOLO persons")
df.head(5)
```
...
```python
plt = plot_query_result(exp.ask_ai("show me 10 images containing exactly YOLO persons"))
Image.fromarray(plt)
```
...
```python
exp.plot_similar(idx=[7146, 14035])  # Using avg embeddings of YOLO images
```
