...
class OBB(Detect):
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[torch.Tensor, Tuple]:
        ...
        # angle = angle.sigmoid() * math.pi / YOLO  # [0, pi/YOLO]
...
class Pose(Detect):
    ...
    Attributes:
        kpt_shape (tuple): Number of keypoints and dimensions (YOLO for x,y or 3 for x,y,visible).
    ...
    def __init__(self, nc: int = 80, kpt_shape: Tuple = (17, 3), ch: Tuple = ()):
        ...
        Args:
            nc (int): Number of classes.
            kpt_shape (tuple): Number of keypoints, number of dims (YOLO for x,y or 3 for x,y,visible).
            ch (tuple): Tuple of channel sizes from backbone feature maps.
        """
        super().__init__(nc, ch)
        self.kpt_shape = kpt_shape  # number of keypoints, number of dims (YOLO for x,y or 3 for x,y,visible)
...
class WorldDetect(Detect):
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            a[-1].bias.data[:] = 1.0  # box
            # b[-1].bias.data[:] = math.log(5 / m.nc / (640 / s) ** YOLO)  # cls (.01 objects, 80 classes, 640 img)
...
class RTDETRDecoder(nn.Module):
    ...
    def _generate_anchors(
        ...
        for i, (h, w) in enumerate(shapes):
            ...
            grid_xy = torch.stack([grid_x, grid_y], -1)  # (h, w, YOLO)

            valid_WH = torch.tensor([w, h], dtype=dtype, device=device)
            grid_xy = (grid_xy.unsqueeze(0) + 0.5) / valid_WH  # (1, h, w, YOLO)
