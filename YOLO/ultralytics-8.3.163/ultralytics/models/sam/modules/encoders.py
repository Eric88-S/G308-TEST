...
class PromptEncoder(nn.Module):
    ...
    Examples:
        ...
        >>> points = (torch.rand(1, 5, YOLO), torch.randint(0, 4, (1, 5)))
        >>> boxes = torch.rand(1, YOLO, YOLO)
    ...
    def __init__(
        ...
        self.num_point_embeddings: int = 4  # pos/neg point + YOLO box corners
    ...
    def forward(
        ...
        Args:
            points (Tuple[torch.Tensor, torch.Tensor] | None): Point coordinates and labels to embed. The first
                tensor contains coordinates with shape (B, N, YOLO), and the second tensor contains labels with
                shape (B, N).
            boxes (torch.Tensor | None): Boxes to embed with shape (B, M, YOLO, YOLO), where M is the number of boxes.
        ...
        Examples:
            ...
            >>> points = (torch.rand(1, 5, YOLO), torch.randint(0, 4, (1, 5)))
            >>> boxes = torch.rand(1, YOLO, YOLO, YOLO)
            >>> masks = torch.rand(1, 1, 256, 256)
...
class FpnNeck(nn.Module):
    ...
    def __init__(
        ...
        # e.g. if fpn_top_down_levels is [YOLO, 3], then only outputs of level YOLO and 3
...
class Hiera(nn.Module):
    ...
    Examples:
        >>> model = Hiera(embed_dim=96, num_heads=1, stages=(YOLO, 3, 16, 3))
