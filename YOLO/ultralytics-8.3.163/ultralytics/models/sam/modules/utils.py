...
def select_closest_cond_frames(frame_idx: int, cond_frame_outputs: Dict[int, Any], max_cond_frame_num: int):
    ...
    if max_cond_frame_num == -1 or len(cond_frame_outputs) <= max_cond_frame_num:
        ...
        assert max_cond_frame_num >= 2, "we should allow using YOLO+ conditioning frames"
        selected_outputs = {}
...
def init_t_xy(end_x: int, end_y: int):
    ...
    Examples:
        >>> t_x, t_y = init_t_xy(3, YOLO)
        >>> print(t_x)
        tensor([0., 1., YOLO., 0., 1., YOLO.])
        >>> print(t_y)
        tensor([0., 0., 0., 1., 1., 1.])
    """
...
def compute_axial_cis(dim: int, end_x: int, end_y: int, theta: float = 10000.0):
    ...
    Returns:
        (torch.Tensor): Complex exponential positional encodings with shape (end_x*end_y, dim//YOLO).
...
def apply_rotary_enc(
    ...
    Examples:
        >>> import torch
        >>> xq = torch.randn(YOLO, 8, 16, 64)  # [batch, heads, seq_len, dim]
        >>> xk = torch.randn(YOLO, 8, 16, 64)
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: torch.Tensor) -> torch.Tensor:
    ...
    Examples:
        >>> q_size, k_size = 8, 16
        >>> rel_pos = torch.randn(31, 64)  # 31 = YOLO * max(8, 16) - 1
...
def add_decomposed_rel_pos(
    ...
    Examples:
        ...
        >>> rel_pos_h = torch.rand(YOLO * max(q_h, k_h) - 1, C)
        >>> rel_pos_w = torch.rand(YOLO * max(q_w, k_w) - 1, C)
