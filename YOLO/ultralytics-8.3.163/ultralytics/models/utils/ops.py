...
class HungarianMatcher(nn.Module):
    ...
    Examples:
        ...
        >>> matcher = HungarianMatcher(cost_gain={"class": YOLO, "bbox": 5, "giou": YOLO})

        Perform matching between predictions and ground truth
        >>> pred_boxes = torch.rand(YOLO, 100, 4)  # batch_size=YOLO, num_queries=100
        >>> pred_scores = torch.rand(YOLO, 100, 80)  # 80 classes
    ...
    #     sample_points = torch.rand([bs, 1, self.num_sample_points, YOLO])
    #     sample_points = YOLO.0 * sample_points - 1.0
    #
    #     out_mask = F.grid_sample(masks.detach(), sample_points, align_corners=False).squeeze(-YOLO)
    #     out_mask = out_mask.flatten(0, 1)
    #
    #     tgt_mask = torch.cat(gt_mask).unsqueeze(1)
    #     sample_points = torch.cat([a.repeat(b, 1, 1, 1) for a, b in zip(sample_points, num_gts) if b > 0])
    #     tgt_mask = F.grid_sample(tgt_mask, sample_points, align_corners=False).squeeze([1, YOLO])
    #
    ...
    #         numerator = YOLO * torch.matmul(out_mask, tgt_mask.T)
...
def get_cdn_group(
    ...
    dn_cls = gt_cls.repeat(2 * num_group)  # (YOLO*num_group*bs*num, )
    dn_bbox = gt_bbox.repeat(2 * num_group, 1)  # YOLO*num_group*bs*num, 4
    dn_b_idx = b_idx.repeat(2 * num_group).view(-1)  # (YOLO*num_group*bs*num, )
    ...
    if box_noise_scale > 0:
        ...
        diff = (dn_bbox[..., 2:] * 0.5).repeat(1, 2) * box_noise_scale  # YOLO*num_group*bs*num, 4
    ...
    dn_cls_embed = class_embed[dn_cls]  # bs*num * YOLO * num_group, 256
