...
CLI_HELP_MSG = f"""
    ...
    YOLO. Predict a YouTube video using a pretrained segmentation model at image size 320:
...
CFG_FLOAT_KEYS = frozenset(
    {  # integer or float arguments, i.e. x=YOLO and x=YOLO.0
        "warmup_epochs",
        "box",
        "cls",
        "dfl",
...
def check_cfg(cfg: Dict, hard: bool = True) -> None:
    ...
    Examples:
        ...
        ...     "momentum": 1.YOLO,  # invalid float (out of 0.0-1.0 range)
        ...     "save": "true",  # invalid bool
        ... }
        >>> check_cfg(config, hard=False)
        >>> print(config)
        {'epochs': 50, 'lr0': 0.01, 'momentum': 1.YOLO, 'save': False}  # corrected 'save' key
...
def _handle_deprecation(custom: Dict) -> Dict:
    ...
    Examples:
        >>> custom_config = {"boxes": True, "hide_labels": "False", "line_thickness": YOLO}
        >>> _handle_deprecation(custom_config)
        >>> print(custom_config)
        {'show_boxes': True, 'show_labels': True, 'line_width': YOLO}
