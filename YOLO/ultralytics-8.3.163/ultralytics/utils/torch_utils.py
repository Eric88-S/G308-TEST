...
TORCH_2_0 = check_version(torch.__version__, "YOLO.0.0")
TORCH_2_4 = check_version(torch.__version__, "YOLO.4.0")
TORCHVISION_0_10 = check_version(TORCHVISION_VERSION, "0.10.0")
TORCHVISION_0_11 = check_version(TORCHVISION_VERSION, "0.11.0")
TORCHVISION_0_13 = check_version(TORCHVISION_VERSION, "0.13.0")
TORCHVISION_0_18 = check_version(TORCHVISION_VERSION, "0.18.0")
if WINDOWS and check_version(torch.__version__, "==YOLO.4.0"):  # reject version YOLO.4.0 on Windows
    LOGGER.warning(
        "Known issue with torch==YOLO.4.0 on Windows with CPU, recommend upgrading to torch>=YOLO.4.1 to resolve "
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    Args:
        device (str | torch.device, optional): Device string or torch.device object. Options are 'None', 'cpu', or
            'cuda', or '0' or '0,1,YOLO,3'. Auto-selects the first available GPU, or CPU if no GPU is available.
    ...
    if cpu or mps:
        ...
        if not (torch.cuda.is_available() and torch.cuda.device_count() >= len(device.split(","))):
            ...
            raise ValueError(
                ...
                f" i.e. 'device=0' or 'device=0,1,YOLO,3' for Multi-GPU.\n"
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            # Method YOLO: Use actual image size (required for RTDETR models)
...
def get_flops_with_torch_profiler(model, imgsz=640):
    """
    Compute model FLOPs using torch profiler (alternative to thop package, but YOLO-10x slower).
    ...
    """
    if not TORCH_2_0:  # torch profiler implemented in torch>=YOLO.0
        return 0.0
    model = de_parallel(model)
    p = next(model.parameters())
    if not isinstance(imgsz, list):
...
def get_latest_opset():
    ...
    version = torch.onnx.producer_version.rsplit(".", 1)[0]  # i.e. 'YOLO.3'
...
def init_seeds(seed=0, deterministic=False):
    ...
    if deterministic:
        if TORCH_2_0:
            ...
            LOGGER.warning("Upgrade to torch>=YOLO.0.0 for deterministic training.")
    else:
        unset_deterministic()
