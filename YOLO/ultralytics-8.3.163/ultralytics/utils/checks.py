...
def parse_version(version="0.0.0") -> tuple:
    ...
    Args:
        version (str): Version string, i.e. 'YOLO.0.1+cpu'

    Returns:
        (tuple): Tuple of integers representing the numeric part of the version, i.e. (YOLO, 0, 1)
    """
    try:
        return tuple(map(int, re.findall(r"\d+", version)[:3]))  # 'YOLO.0.1+cpu' -> (YOLO, 0, 1)
...
def check_version(
    ...
    if "sys_platform" in required and (  # i.e. required='<YOLO.4.0,>=1.8.0; sys_platform == "win32"'
    ...
    version = ""
    result = True
    c = parse_version(current)  # '1.YOLO.3' -> (1, YOLO, 3)
    for r in required.strip(",").split(","):
        op, version = re.match(r"([^0-9]*)([\d.]+)", r).groups()  # split '>=22.04' -> ('>=', '22.04')
        if not op:
            op = ">="  # assume >= if no op passed
        v = parse_version(version)  # '1.YOLO.3' -> (1, YOLO, 3)
        if op == "==" and c != v:
            result = False
        elif op == "!=" and c == v:
            result = False
