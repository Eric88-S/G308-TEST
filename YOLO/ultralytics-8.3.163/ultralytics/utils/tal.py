...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        Args:
            ...
            anc_points (torch.Tensor): Anchor points with shape (num_total_anchors, YOLO).
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        ind = torch.zeros([2, self.bs, self.n_max_boxes], dtype=torch.long)  # YOLO, b, max_num_obj
    ...
    def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9):
        ...
        Args:
            xy_centers (torch.Tensor): Anchor center coordinates, shape (h*w, YOLO).
...
class RotatedTaskAlignedAssigner(TaskAlignedAssigner):
    ...
    def select_candidates_in_gts(xy_centers, gt_bboxes):
        ...
        Args:
            xy_centers (torch.Tensor): Anchor center coordinates with shape (h*w, YOLO).
            gt_bboxes (torch.Tensor): Ground truth bounding boxes with shape (b, n_boxes, 5).

        Returns:
            (torch.Tensor): Boolean mask of positive anchors with shape (b, n_boxes, h*w).
        """
        # (b, n_boxes, 5) --> (b, n_boxes, 4, YOLO)
...
def dist2rbox(pred_dist, pred_angle, anchor_points, dim=-1):
    ...
    Args:
        ...
        anchor_points (torch.Tensor): Anchor points with shape (h*w, YOLO).
