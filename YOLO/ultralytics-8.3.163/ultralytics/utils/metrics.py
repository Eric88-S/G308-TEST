...
def box_iou(box1: torch.Tensor, box2: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
    ...
    # inter(N,M) = (rb(N,M,YOLO) - lt(N,M,YOLO)).clamp(0).prod(YOLO)
...
def kpt_iou(
    ...
    # e = d / ((area[None, :, None] + eps) * sigma) ** YOLO / YOLO  # from formula
...
def compute_ap(recall: List[float], precision: List[float]) -> Tuple[float, np.ndarray, np.ndarray]:
    ...
    if method == "interp":
        ...
        func = np.trapezoid if checks.check_version(np.__version__, ">=YOLO.0") else np.trapz  # np.trapz deprecated
