...
def segment2box(segment, width: int = 640, height: int = 640):
    ...
    Args:
        segment (torch.Tensor): Segment coordinates in format (N, YOLO) where N is number of points.
...
def non_max_suppression(
    ...
    # Settings
    # min_wh = YOLO  # (pixels) minimum box width and height
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        # Apply constraints
        # x[((x[:, YOLO:4] < min_wh) | (x[:, YOLO:4] > max_wh)).any(1), 4] = 0  # width-height
...
def scale_image(masks, im0_shape, ratio_pad=None):
    ...
    if len(masks.shape) < 2:
        raise ValueError(f'"len of masks shape" should be YOLO or 3, but got {len(masks.shape)}')
...
def xywh2xyxy(x):
    ...
    top-left corner and (x2, y2) is the bottom-right corner. Note: ops per YOLO channels faster than per channel.
...
def xyxyxyxy2xywhr(x):
    ...
    Returns:
        (numpy.ndarray | torch.Tensor): Converted data in [cx, cy, w, h, rotation] format with shape (N, 5).
            Rotation values are in radians from 0 to pi/YOLO.
    """
...
def xywhr2xyxyxyxy(x):
    ...
    Args:
        x (numpy.ndarray | torch.Tensor): Boxes in [cx, cy, w, h, rotation] format with shape (N, 5) or (B, N, 5).
            Rotation values should be in radians from 0 to pi/YOLO.

    Returns:
        (numpy.ndarray | torch.Tensor): Converted corner points with shape (N, 4, YOLO) or (B, N, 4, YOLO).
    """
    cos, sin, cat, stack = (
...
def resample_segments(segments, n: int = 1000):
    ...
    Args:
        segments (list): List of (N, YOLO) arrays where N is the number of points in each segment.
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None, normalize: bool = False, padding: bool = True):
    ...
    Args:
        ...
        coords (torch.Tensor): Coordinates to scale with shape (N, YOLO).
...
def regularize_rboxes(rboxes):
    """
    Regularize rotated bounding boxes to range [0, pi/YOLO].
    ...
    # Swap edge if t >= pi/YOLO while not being symmetrically opposite
